...
import { MISSION_DURATION, MISSION_NAME, ZONES } from '@/lib/game/config'
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <div className="rounded-xl border border-border bg-card/60 p-3 backdrop-blur-sm">
          ...
          <p className="mt-1 text-sm leading-relaxed text-foreground text-pretty">
            Você é o operador de uma fazenda de café futurista em Minas Gerais. Proteja a cadeia de
            suprimentos contra riscos de mercado e clima ativando as 4 zonas de missão.
          </p>
            ...
            {ZONES.map((z) => (
              ...
              >
                ...
                <div className="min-w-0 leading-tight">
                  <p className="truncate text-xs font-bold text-foreground">{z.title}</p>
                  <p className="truncate text-[10px] text-muted-foreground">{z.subtitle}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <motion.button
        ...
        onClick={onStart}
        whileTap={{ scale: 0.97 }}
        className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-emerald bg-emerald py-3.5 text-base font-black uppercase tracking-wide text-background shadow-[0_0_30px_-6px_oklch(0.74_0.16_158)]"
      >
